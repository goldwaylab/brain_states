function [ts_file_list, ordered_files_count] = get_files_list(basedir, tokens, PAIRED_RUNS)
    if PAIRED_RUNS
        [ts_file_list, ordered_files_count] = get_paired_files_list(basedir, tokens);
    else
        [ts_file_list, ordered_files_count] = get_plain_files_list(basedir, tokens);
    end
end
function  [ts_file_list, ordered_files_count] = get_paired_files_list(basedir, tokens)
    ts_file_list_1_ = [];
    ts_file_list_2_ = [];
...
function [ts_file_list, ordered_files_count] = get_plain_files_list(basedir, tokens)
        ts_file_list = [];
        ordered_files_count = [];
        for i = 1:size(tokens,1)
            ts_file_list_i = dir(fullfile(basedir, [tokens{i}, '*.csv']));
            ts_file_list = [ts_file_list; ts_file_list_i];
            ordered_files_count = [ordered_files_count, size(ts_file_list, 1)];            
        end
end 
