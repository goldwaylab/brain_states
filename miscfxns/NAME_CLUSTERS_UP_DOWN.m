function [clusterNamesUp,clusterNamesDown,net7angle_Up,net7angle_Down] = NAME_CLUSTERS_UP_DOWN(centroids, atlas)
...
%}
%Taly
[~, ~, file_extension] = fileparts(atlas);
if strcmpi(file_extension, '.mat')
    network7labels = load(atlas).subnetworks_reorder;
else
    network7labels_file = readtable(atlas);
    network7labels = network7labels_file.chosen_nw_fixed;
end
network7labels = horzcat(network7labels,network7labels,network7labels,network7labels,network7labels,network7labels,network7labels);
